...
public class ServiceOSDAO {
    ...
    public ServiceOSModel buscarOSPorId(int id) {
        System.out.println("🔍 Buscando OS ID: " + id);
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (rs.next()) {
                System.out.println("✅ OS encontrada! ID: " + rs.getInt("id"));

                // Criando objeto da OS
                ServiceOSModel os = new ServiceOSModel(
                    rs.getInt("id"),
                    rs.getInt("cliente_id"),  
                    rs.getInt("carro_id"),    
                    rs.getString("tipoOS") != null && !rs.getString("tipoOS").isEmpty() ? rs.getString("tipoOS") : "Indefinido",
                    ...
                    rs.getString("situacao") != null && !rs.getString("situacao").isEmpty() ? rs.getString("situacao") : "Aberta",
                    rs.getTimestamp("data_entrada") != null ? rs.getTimestamp("data_entrada").toLocalDateTime().toLocalDate() : LocalDate.now(),
                    rs.getDate("data_previsao") != null ? rs.getDate("data_previsao").toLocalDate() : null
                );
                ...
                System.out.println("❌ Nenhuma OS encontrada para o ID " + id);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public List<CarPartsModel> buscarPecasPorOS(int osId) {
	    ...
	    String sql = "SELECT p.id, p.nome, p.marca, p.valorVenda " +
	                 "FROM ordens_servico_pecas op " +
	    ...
	    try (Connection conn = DatabaseConnection.getConnection();
	        ...
	        while (rs.next()) {
	            CarPartsModel peca = new CarPartsModel(
	                ...
	                rs.getString("nome"),
	                rs.getString("marca"),
	                rs.getBigDecimal("valorVenda") // Certifique-se de que o tipo é compatível
	            );
	            pecas.add(peca);
	        }
	    } catch (SQLException e) {
	...
	public int salvarOrdemServico(ServiceOSModel os) {
	    ...
	    try (Connection conn = DatabaseConnection.getConnection();
	        ...
	        if (affectedRows == 0) {
	            throw new SQLException("Falha ao salvar OS, nenhuma linha afetada.");
	        }
	        ...
	        try (ResultSet generatedKeys = stmt.getGeneratedKeys()) {
	            if (generatedKeys.next()) {
	                ...
	                throw new SQLException("Falha ao obter o ID gerado da OS.");
	            }
	        }
    ...
    public int salvarOrdemServicoComPecas(ServiceOSModel os) {
        int osId = salvarOrdemServico(os); // Primeiro, salva a OS e obtém o ID gerado

        if (osId > 0) {
            salvarPecasOrdemServico(osId, os.getPecas()); // Passa a lista original de peças
        } else {
            System.err.println("ERRO: A OS não foi salva corretamente. Nenhum ID gerado.");
        }
	...
	public void salvarPecasOrdemServico(int osId, List<CarPartsModel> pecas) {
	    if (osId <= 0) {
	        System.err.println("❌ ERRO: ID da OS inválido ao salvar peças.");
	        return;
	    }
	    ...
	    try (Connection conn = DatabaseConnection.getConnection();
	        ...
	        System.out.println("✅ Peças atualizadas para OS ID " + osId);
